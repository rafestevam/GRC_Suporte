...
public class CustomRisk2TestDefinitionHandler implements IViewHandler{
	...
	public void handleView(QueryDefinition query, List<IRightsFilterCriteria> rightsFilters,
		...
		List<Long> tdIDList = new ArrayList();
			...
			if(currentObject.getObjMetaData().getObjectType().getId().equalsIgnoreCase("RISK")){
				...
				for (IAppObj controlObj : listCtrlID) {
					...
					for (IAppObj tdAppObj : testDefList) {
						if(tdAppObj.getVersionData().isHeadRevision())
							tdIDList.add(tdAppObj.getObjectId());
				...
				if(tdIDList.size() > 0)
					filters.add(new SimpleFilterCriteria("testdefinition_obj_id", DataLayerComparator.IN, tdIDList));
