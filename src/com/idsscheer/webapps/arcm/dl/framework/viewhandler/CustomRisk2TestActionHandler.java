...
public class CustomRisk2TestActionHandler implements IViewHandler{
	...
	public void handleView(QueryDefinition query, List<IRightsFilterCriteria> rightsFilters,
		...
		List<Long> tcIDList = new ArrayList();
		...
		try {
			...
			if(currentObject.getObjMetaData().getObjectType().getId().equalsIgnoreCase("RISK")){
				...
				if(tcIDList.size() > 0)
					filters.add(new SimpleFilterCriteria("ta_id", DataLayerComparator.IN, tcIDList));
