...
public class CustomHierarchyForm extends Form {
	...
	protected void renderButtons(List<IButtonType> activeButtonTypes) {
		...
		// IEnumerationItem role =
		// ARCMCollections.extractSingleEntry(riskManagerGrp.getAttribute(IUsergroupAttributeType.ATTR_ROLE).getRawValue(),
		// true);
		...
		/*
		 * if(!this.form.getRoleSelectionModel().getSelectedRole().getId().
		 * equalsIgnoreCase(role.getId()))
		 * activeButtonTypes.remove(ButtonTypesCustom.CUSTOM_CR_EVALUATION);
		 * 
		 * if(this.form.getRoleSelectionModel().getSelectedRole().getId().
		 * equalsIgnoreCase(role.getId()))
	...
	private IAppObj loadRiskManagerGrp() {
		...
		IAppObjFacade facade = FacadeFactory.getInstance().getAppObjFacade(this.environment.getUserContext(),
	...
	protected void renderPage(IPage page) {
		...
		try {
						...
						for (IRow row : rowGroup.getRows()) {
							if (row.getId().equals("cst_rskcateg")) {
								...
								this.buffer.append("<tr id=\"tr_cst_rskcateg\" class=\"ATTRIBUTE_ROW contentfont \">")
										...
										.append(categRC).append("</td></tr></tbody></table>").append("</td>")
										.append("</tr>");
							}
						}
					}
			...
			this.environment.getDialogManager().getNotificationDialog()
					.addError("Erro ao obter Categoria de Risco " + e.getMessage());
		}
	...
	private String getCorpRiskCateg(IAppObj rcObj) throws Exception {
		...
		IViewQuery query = QueryFactory.createQuery(this.environment.getUserContext(), "cst_view_corprisk", filterMap,
				null, true, defaultTransaction);
		...
		try {
			...
			while (itQuery.hasNext()) {
				...
				/*
				 * long categID = (Long)viewObj.getRawValue("ct_obj_id"); long
				 * categVersionNumber =
				 * (Long)viewObj.getRawValue("ct_version_number");
				 * 
				 * IOVID categOVID = OVIDFactory.getOVID(categID,
				 * categVersionNumber); IAppObj categAppObj =
				 * this.environment.getAppObjFacade(ObjectType.HIERARCHY).load(
				 * categOVID, defaultTransaction, true);
				 */
